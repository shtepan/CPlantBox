...
maxS = 4  # maximal
minS = 0.7  # minimal
slope = 15  # linear gradient between min and max (cm), half length of linear interpolation between fmax and fmin
box = pb.SDF_PlantBox(30, 60, 40)  # cm
layer = pb.SDF_RotateTranslate(box, pb.Vector3d(0, 0, -18))
...
for i in range(0, C):
    for j in range(0, R):
        ...
        sigma = [0.4, 0.4, 0.5]
        tropN = [0.7, 0.8, 1.0] # strength of tropism
...
for i, rs in enumerate(allRS):
      vtpname = "results/example_LAB_multiple_hydrotrop_201002b" + str(i) + ".vtp"
      rs.write(vtpname)
